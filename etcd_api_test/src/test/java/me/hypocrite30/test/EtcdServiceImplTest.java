...
/**
 * @Author: Hypocrite30
 * @Date: 2021/11/24 22:35
 */
@TestMethodOrder(MethodOrderer.OrderAnnotation.class)
public class EtcdServiceImplTest {

    private static EtcdService etcdService = new EtcdServiceImpl("http://192.168.248.128:2379,http://192.168.248.128:2380,http://192.168.248.128:2381");
    ...
    /**
     * 调用put方法传入key和value，至于验证，在开始读操作之前先简单点，确认header非空即可
     */
    @Test
    @Order(1)
    @DisplayName("基本写操作")
    ...
    /**
     * 借助 GetOption 对象，我们可以进行跟多复杂的读操作
     */
    @Test
    @Order(3)
    ...
    /**
     * revision 字段是 etcd的全局版本号，每次写入都会对应一个 revision 值，可以用该 revision 值作为查询条件，查到指定 key 过往的某个版本的值
     */
    @Test
    @Order(5)
    ...
    /**
     * 假设etcd有三个key：a1、a2、a3，那么通过前缀a可以将这三个key都查出来，与此同时还可以再加个endKey查询条件，
     * 假设endKey等于a2，那么查找工作在查到a2时就会停止并返回，而返回值中只有a1，不包含a2，换言之endKey之前的值才会被返回
     */
    @Test
    @Order(9)
    ...
    /**
     * 借助 DeleteOption 对象，可以实现更多类型的删除，下面是删除指定前缀的所有记录
     */
    @Test
    @Order(11)
    @DisplayName("删除(指定前缀)")
    ...
    /**
     * 与读操作的endKey类似，删除操作也有endKey参数，假设etcd有三个key：a1、a2、a3，那么通过前缀a可以将这三个key都删除，
     * 与此同时还可以再加个endKey删除条件，假设endKey等于a2，那么删除工作在查到a2时就会停止并返回，被删除的记录只有a1，
     * 不包含a2，换言之endKey之前的记录才会被删除
     * @throws Exception
     */
    @Test
    @Order(11)
